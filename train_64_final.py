"""Train with 64-char ASCII tokenizer (space to underscore, no corruption)."""
import os, sys, time, json, importlib
import numpy as np

# --- 64-char ASCII tokenizer (no corruption) ---
# ASCII 32-95 = 64 chars: ' ' to '_'
# All lowercase a-z ARE included (ASCII 97-122 -> IDs 65-90)
VOCAB_64 = ''.join(chr(32 + i) for i in range(64))  # ' ' through '_'
VOCAB_SIZE = 64
...
# Patch model.py directly
model_path = 'train/model.py'
with open(model_path, 'r') as f:
    model_src = f.read()

# Build the 64-char vocab as Python expression
vocab_expr = "[chr(32+i) for i in range(64)]"
old_block = """VOCAB_SIZE = 95
VOCAB = ''.join(chr(32 + i) for i in range(VOCAB_SIZE))  # ' ' to '~'"""
new_block = """VOCAB_SIZE = 64
VOCAB = ''.join(chr(32 + i) for i in range(VOCAB_SIZE))  # ' ' to '_'"""

assert old_block in model_src, f"Could not find old block in model.py!"
model_src = model_src.replace(old_block, new_block)

# Write patched model
patched_path = 'train/model_64.py'
...
import model_64 as _m
importlib.reload(_m)

from model_64 import init_weights, forward_full, backward_full, param_size
from model_64 import VOCAB_SIZE as VS, VOCAB as V, D_MODEL, N_LAYERS, N_HEADS, HEAD_DIM, D_FF, CTX_LEN

print(f"Model: vocab={VS}, d_model={D_MODEL}, layers={N_LAYERS}, heads={N_HEADS}, d_ff={D_FF}, ctx={CTX_LEN}")
sys.stdout.flush()

# Verify model uses same VOCAB
print(f"Model VOCAB length: {len(V)}, starts: {repr(V[:5])}, ends: {repr(V[-3:])}")
assert VS == 64, f"Model VOCAB_SIZE is {VS}, not 64!"
...
# AdamW step (copied from train.py)
